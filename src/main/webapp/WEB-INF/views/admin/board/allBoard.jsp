...
<script src="<c:url value='/js/loadDomBoardAdmin.js'/>"></script>
</head>
<body>
<!--  회원정보가 필요하다 -->
<label id="loginMember" style="display:none;" >${loginMember.uid}</label>
<label id="boardUrl" style="display:none;" ><c:url value="/board/AllBoard.do"/></label>
<label id="loginUrl" style="display:none;" ><c:url value="/member/loginForm.do"/></label>
...
<label id="board-id" style="display:none;">${board.boardid}</label>
	<table class="post">
		<tbody id="view-table">
			...
			<tr>
				<th>내용</th>
				<td colspan="5"><textarea id="board-contents" cols="60" rows="10" readonly>${board.contents}</textarea></td>
			</tr>
	...
	<!-- 댓글 입력폼 시작-->
	 <form id="insertReply" action="<c:url value='/reply/InsertReply.do'/>" method="post" onSubmit="return false;">
          ...
          <input id="insert-userid" name="memberid" value="${loginMember.uid}" readonly>
            <textarea id="insert-comments" form="insertReply" class="comment" name="comment" cols="60" rows="5"></textarea>
	...
	<!-- 댓글 입력폼 끝 -->
        ...
        <div>
			<input type="button" id="moreBtn" value="더보기" style="width: 100%;border: none;
			background-color: #d36f12b3;color: white;height: 25px;margin-top: 10px;border-radius: 5px;"/>
		</div>
	...
	<form action="<c:url value='/board/DeleteSelectPage.do'/>" method='post' onsubmit="return false;">
		...
		<tr >
			 <th > <input type='checkbox' class='boardid' name='boardid' value='${board.boardid}'></th>
				...
				<td ><button class="deleteOne" data-server-id="${board.boardid }">삭제</button></td>
			</tr>
		...
		<a id="registerBoard">게시글작성</a>
		
		<input type='submit' value="삭제" onclick="jsSelectedDelete()">
		...
		function jsCheckbox(){
			...
			function jsSelectedDelete(){
			    ...
			    const param = {
				      ...
				      fetch("<c:url value='/admin/board/DeleteSelectPage.do'/>", {
				        method: "POST",
				        headers: {
				          ...
				          if (json.status) {
				        	  const boardList = json.boardList;
				                ...
				                for(let i=0;i<boardList.length;i++){
				                	...
				                	boardInfo += "<tr >";
				                	boardInfo += "<th><input type='checkbox' class='boardid' name='boardid' value='"+board.boardid+"'></th>";
				                	boardInfo += "<td class='kk'>"+board.boardid+"</td>";
				                	boardInfo += "<td class='viewDetail' data-server-id='"+board.boardid+"' style='text-align: left; padding-left:"+(board.level-1)*20+"px;width: 25%;'>" 
				                	boardInfo += (board.level != 1)? "ㄴ[답변]":"" 
				                	boardInfo +=   board.title+"</td>";
				                	boardInfo += "<td>"+board.hit+"</td>";
				                	boardInfo += "<td>"+board.memberid+"</td>";
				                	boardInfo += "<td ><button class='deleteOne' data-server-id='"+board.boardid+"'>삭제</button></td>";
				                	boardInfo += "</tr>";
				                	
				                	$('#boardList').html($('#boardList').html()+boardInfo);
				                }
				          }
				      });
			...
			$(".deleteOne").click(function(){
				...
				$.ajax({
					url:"<c:url value='/admin/board/DeleteSelectPage.do'/>",
					method: "POST",
